		...
		<div class="row mb-2">
			...
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right text-sm">
					...
					<li class="breadcrumb-item active">Ventures</li>
				</ol>
			</div><!-- /.col -->
		</div><!-- /.row -->
	</div><!-- /.container-fluid -->
			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h3 class="card-title">Ventures</h3>
						...
						<table id="indextable" class="table table-bordered table-hover table-sm text-sm">
							...
							<tbody>
								...
								<tr>
									...
									<td>{{ $v->name }}</td>
									<td> {{ $v->description}}</td>
									<td><a href="{{ route('admin.ventures.edit',$v->id) }}" class="btn btn-info btn-xs">Edit</a> 
										{{-- <a href="javascript:void(0)" onclick="$(this).parent().find('form').submit()" class="btn btn-danger btn-xs">Delete</a>
										<form action="{{route('admin.ventures.destroy',$v->id) }}" method="post">
											@method('DELETE')
								...
								@endforeach
								@else
								<tr><td colspan="5">No ventures</td></tr>
								@endif
							</tbody>
						</table>
						...
						<div class="modal-dialog modal-dialog-centered modal-lg">
							<div class="modal-content">
								<div class="modal-header">
									<h3 class="modal-title">Add venture</h3>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
										<span aria-hidden="true">&times;</span>
									</button>
								</div>
								<div class="modal-body">
									<form method="post" action="{{ route('admin.ventures.store') }}">
											<input type="hidden" name="_token" value="{{ csrf_token() }}" >
